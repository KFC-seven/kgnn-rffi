...
from kgnn.supcon_loss import supervised_contrastive_loss
...
class Ossei2025NearFullModel(nn.Module):
    def __init__(
        self,
        *,
        model_name: str,
...
def _evaluate_accuracy(model: Ossei2025NearFullModel, loader: DataLoader, *, device: str) -> float:
    model.eval()
    total = 0
    correct = 0
    with torch.no_grad():
